...
export default function UnderstandClient({ initialSessions }: UnderstandClientProps) {
  ...
  const handleSessionCreated = (sessionId: string) => {
    ...
    router.push(buildUrl(`/modules/understand/${sessionId}`));
  };
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        {permissions.canViewAllResidents ? (
          <button
            onClick={() => router.push(buildUrl('/modules/understand/residents'))}
          ...
          <button
            onClick={() => router.push(buildUrl('/modules/understand/my-profile'))}
            className="group bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg border border-white/50 p-6 text-left hover:shadow-xl hover:bg-white/80 transition-all"
          >
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[#4A90A8]/20 to-[#0EA5E9]/20 flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        {/* Class Cohort Card */}
        <button
          onClick={() => router.push(buildUrl('/modules/understand/class'))}
          className="group bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg border border-white/50 p-6 text-left hover:shadow-xl hover:bg-white/80 transition-all"
        >
          <div className="flex items-start gap-4">
        ...
        {/* Program-Wide Card */}
        <button
          onClick={() => router.push(buildUrl('/modules/understand/program'))}
          className="group bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg border border-white/50 p-6 text-left hover:shadow-xl hover:bg-white/80 transition-all"
        >
          <div className="flex items-start gap-4">
          ...
          {sessions.length === 0 ? (
            ...
            <div className="divide-y divide-neutral-100">
              {sessions.map((session) => (
                <button
                  key={session.id}
                  onClick={() => router.push(buildUrl(`/modules/understand/${session.id}`))}
