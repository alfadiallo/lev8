...
2. Check that `health_systems`, `programs`, and `module_buckets` tables have data
3. Verify `voice_journal` storage bucket exists in Storage section

### Check user profiles:
1. Go to Supabase Dashboard > Authentication > Users
2. Verify your user exists
3. Go to Table Editor > `user_profiles` - verify your profile exists
4. Go to Table Editor > `residents` - verify your resident record exists (if you're a resident)

### Check database schema:
1. Go to Supabase Dashboard > SQL Editor
2. Run: `SELECT * FROM programs LIMIT 1;`
3. Should return at least one program record
...
- **Fix:** Check if resident record was created in Supabase Dashboard > Table Editor > `residents`
- **Solution:** Resident record creation might have failed silently - check terminal logs for `[Register]` messages
